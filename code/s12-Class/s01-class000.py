...
class Soldier(AttackUnit):
    def __init__(self):
        AttackUnit.__init__(self, "보병", 40, 5, 1) # 이름, 체력, 공격력, 이동속도
...
class Tank(AttackUnit):
    ...
    def set_siege_mode(self):
        ...
        # 현재 시지 모드일 때
            ...
            print("{0} 시지 모드를 해체합니다.".format(self.name))
            self.damage //= 2 # 공격력 절반으로 감소
            self.siege_mode = False # 시지 모드 해체
...
class Stealth(FlyableAttackUnit):
    def __init__(self):
        ...
        self.cloaked = False # 은페 모드 (해체 상태)
    ...
    def cloaking(self):
        ...
        if self.cloaked == True:
            print("{0} : 은폐 모드를 해체합니다.".format(self.name))
            self.cloaked == False
...
# 공격 모드 준비(보병: 강화제, 탱크: 시저 모드, 전투기: 은페 모드)
for unit in attack_units:
   if isinstance(unit, Soldier): # Soldier 클래스의 인스턴스 이면 강화제
        unit.booster()
   elif isinstance(unit, Tank): # Tank 클래스의 인스턴스이면 시저 모드
        unit.set_siege_mode()
